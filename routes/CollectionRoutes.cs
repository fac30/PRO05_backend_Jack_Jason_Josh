    ...
    public static void MapCollectionRoutes(this WebApplication app)
    {
        ...
        app.MapGet(
                ...
                async (int id, ColourContext context) =>
                {
                    ...
                    var colourCollections = await context
                        .ColourCollections.Where(cc => cc.CollectionId == id)
        ...
        app.MapPost(
                ...
                async (int id, AddColourToCollectionDTO dto, ColourContext context) =>
                {
                    ...
                    var existingEntry = await context.ColourCollections.FirstOrDefaultAsync(cc =>
                        cc.CollectionId == id && cc.ColourId == dto.ColourId
                    );

                    if (existingEntry != null)
                    {
                        return Results.BadRequest(
                            $"Colour {dto.ColourId} is already in collection {id}."
                        );
                    }
        ...
        app.MapPut(
                ...
                async (int id, string type, ColourContext context) =>
                {
                    ...
                    // The following logic should be inside the same closure if it's meant to modify the same collection
                    collection.IsPublic = !collection.IsPublic;
                    await context.SaveChangesAsync();
        ...
        app.MapPut(
                "/collections/{collectionId}/colours/{colourId}/order",
                async (int collectionId, int colourId, int newOrder, ColourContext context) =>
                {
                    ...
                    var collectionColours = await context
                        .ColourCollections.Where(cc => cc.CollectionId == collectionId)
                        .OrderBy(cc => cc.Order)
                        .ToListAsync();
                    ...
                    var colourToMove = collectionColours.FirstOrDefault(cc =>
                    ...
                    if (colourToMove == null)
                    {
                        return Results.NotFound(
                            $"Colour {colourId} not found in collection {collectionId}."
                        );
                    }
        ...
        app.MapDelete(
                ...
                async (int id, ColourContext context) =>
                {
                    ...
                    return Results.Ok(
                        $"Collection '{collection.Name}' and all associated data deleted."
                    );
                }
            )
            .WithTags("Collections");
        ...
        app.MapDelete(
                "/collections/{collectionId}/colours/{colourId}",
                async (int collectionId, int colourId, ColourContext context) =>
                {
                    var colourCollection = await context.ColourCollections.FirstOrDefaultAsync(cc =>
                        cc.CollectionId == collectionId && cc.ColourId == colourId
                    );
                    ...
                    var higherOrderColours = await context
                        .ColourCollections.Where(cc =>
                            cc.CollectionId == collectionId && cc.Order > removedOrder
                        )
                        .ToListAsync();
